...
class Mediameta_model extends MY_Model {
	 ...
	 * @return array|bool
	 */
	public function get_all( $filters = [], $sort = 'meta_id', $dir = 'ASC', $limit = 0, $offset = 0 )
	{
	 ...
	 * @return int|boolean
	 */
	public function create( $media_id = NULL, $meta_key = '', $meta_value = NULL )
	{
		if ( $media_id !== NULL && ! empty( $meta_key ) && ! empty( $meta_value ) )
		{
			$this->db->insert(
				$this->_db,
				[
	 ...
	 * @return boolean|int
	 */
	public function count( $meta_key = NULL, $media_id = NULL ) : bool
	{
		if ( $meta_key && $meta_key )
		{
			$this->db->select( 'COUNT(*) AS count' )
				->from( $this->_db )
				->where( [ 'meta_key' => $meta_key, 'media_id' => $media_id ] );
