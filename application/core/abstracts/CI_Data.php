...
abstract class CI_Data {
	...
	{
		return ci_json_encode( $this->get_data() );
	}
	...
	{
		return 'codeigniter_' . $this->object_type . '_get_';
	}
	...
	{
		return CI_Cache_Helper::get_cache_prefix( $cache_group ) . CI_Cache_Helper::get_cache_prefix( 'object_' . $id ) . 'object_meta_' . $id;
	}
	...
	public static function prime_raw_meta_data_cache( $raw_meta_data_collection, $cache_group )
	{
		foreach ( $raw_meta_data_collection as $object_id => $raw_meta_data_array )
		{
			...
			ci_cache_set( $cache_key, $raw_meta_data_array, $cache_group );
		}
	}
	...
	public function init_meta_data( array $filtered_meta_data = [] )
	{
		...
		{
			$this->meta_data[] = new CI_Meta_Data(
				[
					'id'    => (int) $meta->meta_id,
					'key'   => $meta->meta_key,
	...
	public function read_meta_data( $force_read = FALSE )
	{
		...
		if ( ! $force_read )
		{
			if ( ! empty( $this->cache_group ) )
			{
				$cached_meta = ci_cache_get( $cache_key, $this->cache_group );
		...
		if ( is_array( $raw_meta_data ) )
		{
			...
			{
				ci_cache_set( $cache_key, $raw_meta_data, $this->cache_group );
			}
		}
	}
	...
	{
		$this->maybe_read_meta_data();
		$array_keys = ci_list_pluck( $this->get_meta_data(), 'key' );
	...
	public function save_meta_data()
	{
		...
		if ( ! empty( $this->cache_group ) )
		{
			...
			ci_cache_delete( $cache_key, $this->cache_group );
		}
	}
	...
	public function update_meta_data( $key, $value, $meta_id = 0 )
	{
		...
		if ( $meta_id )
		{
			$array_keys = array_keys( ci_list_pluck( $this->meta_data, 'id' ), $meta_id, TRUE );
	...
	{
		$this->maybe_read_meta_data();
		$array_keys = array_keys( ci_list_pluck( $this->meta_data, 'key' ), $key, TRUE );
	...
	{
		$this->maybe_read_meta_data();
		$array_keys = array_keys( ci_list_pluck( $this->meta_data, 'id' ), (int) $mid, TRUE );
	 ...
	 *
	 * @throws CI_Data_Exception Data Exception.
	 */
	protected function error( $code, $message, $http_status_code = 400, $data = [] )
	{
		throw new CI_Data_Exception( $code, $message, $http_status_code, $data );
	}
	...
	|--------------------------------------------------------------------------
	|
	| Functions for setting data. These should not update anything in the
	...
	public function set_props( $props, $context = 'set' )
	{
		...
		foreach ( $props as $prop => $value )
		{
			try
			{
				 ...
				 */
				if ( is_null( $value ) || in_array( $prop, [ 'meta_data' ], TRUE ) )
				{
					continue;
				}
				$setter = "set_$prop";
	...
	protected function set_date_prop( $prop, $value )
	{
		try
		{
			...
			elseif ( is_numeric( $value ) )
			{
				...
				$datetime = new CI_DateTime( "@{$value}", new DateTimeZone( 'UTC' ) );
			}
			else
			{
				...
				{
					$offset = ! empty( $date_bits[ 7 ] ) ? iso8601_timezone_to_offset( $date_bits[ 7 ] ) : ci_timezone_offset();
					$timestamp = gmmktime( $date_bits[ 4 ], $date_bits[ 5 ], $date_bits[ 6 ], $date_bits[ 2 ], $date_bits[ 3 ], $date_bits[ 1 ] ) - $offset;
				}
				else
				{
					$timestamp = ci_string_to_timestamp( get_gmt_from_date( gmdate( 'Y-m-d H:i:s', ci_string_to_timestamp( $value ) ) ) );
				}
				$datetime = new CI_DateTime( "@{$timestamp}", new DateTimeZone( 'UTC' ) );
			}
			
			// Set local timezone or offset.
			if ( get_option( 'timezone_string' ) )
			{
				$datetime->setTimezone( new DateTimeZone( ci_timezone_string() ) );
			}
			else
			{
				$datetime->set_utc_offset( ci_timezone_offset() );
			}
	...
	*/
	...
	public function get_meta( $key = '', $single = TRUE )
	{
		...
		$array_keys = array_keys( ci_list_pluck( $meta_data, 'key' ), $key, TRUE );
		$value = $single ? '' : [];
