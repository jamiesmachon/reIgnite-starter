...
abstract class REST_Controller extends MY_Controller {
	...
	public function __construct( $config = 'rest' )
	{
		...
		setlocale( LC_ALL, explode( ",", "en_GB, en_GB.iso88591, en_GB.iso885915, en_GB.utf8, english" ) );
		
		$this->output->enable_profiler( FALSE );
		
		$maintenance_mode = $this->settings->maintenance_mode;
		$whitelisted_ip = explode( ",", $this->settings->maintenance_mode_whitelist );
		$scheduled = $this->settings->maintenance_mode_schedule;
		
		if ( $maintenance_mode )
		{
			$this->includes[ "maintenance_mode" ] = TRUE;
			$this->includes[ "maintenance_mode_schedule" ] = $scheduled;
			if ( ( ! empty( $scheduled ) && time() >= strtotime( $scheduled ) ) || empty( $scheduled ) )
			{
				if ( ! in_array( $_SERVER[ "REMOTE_ADDR" ], $whitelisted_ip ) )
				{
					exit;
				}
			}
