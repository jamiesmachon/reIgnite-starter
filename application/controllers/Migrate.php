 ...
 * @subpackage
 * @category
 * @author      Rob Bates & Jamie Machon <support@nextpixel.com>
 * @link
 */
class Migrate extends CI_Controller {
	...
	protected function _customize_migrations_table()
	{
		...
		$fields = [
			'latest_file_version' => [ 'type' => 'BIGINT', 'default' => 0, 'constraint' => 20 ],
		];
	...
	public function index( $file_name = '' )
	{
		...
		$file_name = trim( fgets( STDIN ) ) . ' ' . bin2hex( random_bytes( 10 ) );
	 ...
	 * @return string
	 */
	protected function _get_file_content( $file_name = '', $langs = [] )
	{
		// Php opening tag
		...
		if ( strpos( $this->_migration_file_name, 'update languages' ) === 0 )
		{
			$file_content .= '        if (is_array($this->_langs())) {' . PHP_EOL;
			$file_content .= '            foreach ($this->_langs() as $lang) {' . PHP_EOL;
			$file_content .= '                $sql = $this->db->set($lang)->get_compiled_insert($this->_table_name);' . PHP_EOL;
			$file_content .= '                $sql .= " ON DUPLICATE KEY UPDATE `text` = VALUES(`text`)";' . PHP_EOL;
			$file_content .= '                $this->db->query($sql);' . PHP_EOL;
			$file_content .= '            }' . PHP_EOL;
			$file_content .= '        }' . PHP_EOL;
		}
		elseif ( strpos( $this->_migration_file_name, 'modify' ) === 0 )
		{
		...
		else
		{
			...
			$file_content .= '        $this->dbforge->add_field($this->_fields());' . PHP_EOL;
			$file_content .= '        $this->dbforge->add_field("`created` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP");' . PHP_EOL;
			$file_content .= '        $this->dbforge->add_field("`updated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");' . PHP_EOL;
			$file_content .= '        $this->dbforge->add_field("`delete` timestamp NULL DEFAULT NULL");' . PHP_EOL;
			$file_content .= '        $this->dbforge->create_table($' . 'this->_table_name, TRUE, ["ENGINE" => "InnoDB"]);' . PHP_EOL;
		}
		...
		if ( strpos( $this->_migration_file_name, 'update languages' ) === 0 )
		{
			$file_content .= '        if (is_array($this->_langs())) {' . PHP_EOL;
			$file_content .= '            foreach ($this->_langs() as $lang) {' . PHP_EOL;
			$file_content .= '                $this->db->delete($this->_table_name, [\'key\' => $lang[\'key\'], \'set\' => $lang[\'set\'], \'language\' => $lang[\'language\']]);' . PHP_EOL;
			$file_content .= '            }' . PHP_EOL;
			$file_content .= '        }' . PHP_EOL;
		}
		elseif ( strpos( $this->_migration_file_name, 'modify' ) === 0 )
		...
		if ( strpos( $this->_migration_file_name, 'update languages' ) === 0 )
		{
			$file_content .= PHP_EOL;
			$file_content .= '    /**' . PHP_EOL;
			$file_content .= '     * Returns an array of the language lines to be used within the up and down functions!' . PHP_EOL;
			$file_content .= '     *' . PHP_EOL;
			$file_content .= '     * @return array' . PHP_EOL;
			$file_content .= '     */' . PHP_EOL;
			$file_content .= '    protected function _langs()' . PHP_EOL;
			$file_content .= '    {' . PHP_EOL;
			$file_content .= '        return [' . PHP_EOL;
			foreach ( $langs as $lang )
			{
				$file_content .= '            [' . PHP_EOL;
				$file_content .= '                \'key\'      => \'' . $lang[ 'key' ] . '\',' . PHP_EOL;
				$file_content .= '                \'set\'      => \'' . $lang[ 'set' ] . '\',' . PHP_EOL;
				$file_content .= '                \'language\' => \'' . $lang[ 'language' ] . '\',' . PHP_EOL;
				$file_content .= '                \'text\'     => \'' . $lang[ 'text' ] . '\',' . PHP_EOL;
				$file_content .= '            ],' . PHP_EOL;
			}
			$file_content .= '        ];' . PHP_EOL;
			$file_content .= '    }' . PHP_EOL;
	...
	{
		$file_name = 'update languages ' . bin2hex( random_bytes( 10 ) );
		$migration_file_table = 'languages';
		
		$lang_lines = [];
		$base_path = APPPATH . 'language' . DIRECTORY_SEPARATOR;
		$languages = array_slice( scandir( $base_path ), 2 );
		
		foreach ( $languages as $language )
		{
			$lang_file = $base_path . DIRECTORY_SEPARATOR . $language . DIRECTORY_SEPARATOR . 'dev_lang.php';
			if ( file_exists( $lang_file ) )
			{
				...
				ksort( $fails );
				$reset = '<?php defined( \'BASEPATH\' ) OR exit( \'No direct script access allowed\' );' . PHP_EOL;
				foreach ( $fails as $key => $line )
				{
					$reset .= '$lang[\'' . $key . '\'] = \'' . $line . '\';' . PHP_EOL;
				}
				
				file_put_contents( $lang_file, $reset );
				unset( $fails );
				unset( $lang );
			}
		}
		
		if ( count( $lang_lines ) < 1 )
		{
			return;
		}
		...
		// Get the default migration file content
		$file_content = $this->_get_file_content( $file_name, $lang_lines );
	...
	/**
	 * Migrate to a migration version
	 *
	...
	{
		if ( $version > $this->_get_version( 'version' ) )
		{
		...
		{
			echo "You must provide a version number greater than the current version " . $this->_get_version( 'version' );
		}
	}
	...
	public function last()
	{
		if ( $this->config->item( 'migration_type' ) === 'sequential' )
		{
			$rollback_version = $this->_get_version() == 0 ? 0 : ( $this->_get_version() - 1 );
		}
		else
		{
			...
			if ( ( is_array( $migrations ) ) && ( count( $migrations ) > 0 ) )
			{
				...
				{
					if ( $this->_get_migration_number( $value ) == $version )
					{
						break;
					}
					$rollback_version = $this->_get_migration_number( $value );
				}
			}
		}
	...
	{
		if ( ! file_exists( FCPATH . 'assets/' . $this->_info_file_name ) )
		{
			show_error( "Couldn't find the info file!" );
		}
		
		$file_content = file_get_contents( FCPATH . 'assets/' . $this->_info_file_name );
